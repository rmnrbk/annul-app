...
export class DetailsValidators {
  static prepayPercValidator(percName: string) {
    return (formGroup: FormGroup) => {
      const perc = formGroup.get(percName)?.value;
      if (perc < 0 || perc > 100) {
        return { prepayPercInvalid: true };
      }
  ...
  static prepayAndPayDateRangeValidator(
    prepayDateName: string,
    payDateName: string
  ) {
    return (formGroup: FormGroup) => {
      return this.dateRangeValidator(prepayDateName, payDateName, formGroup)
        ? null
        : { pprepayAndPayDateRangeInvalid: true };
    };
  }
